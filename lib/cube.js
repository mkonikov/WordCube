...
class Cube {
  ...
  rollDice() {
    ...
    for (i = rolledDice.length - 1; i > 0; i -= 1) {
  ...
  shuffleCube() {
    this.cube.innerHTML = "";
    const rolledDice = this.rollDice();
    let id = 0;
    [0, 1, 2, 3, 4].forEach((y) => {
      [0, 1, 2, 3, 4].forEach((x) => {
        const letterOptions = {
          coords: [x, y],
          character: rolledDice[id][Math.floor(Math.random() * 5)]
        };
  ...
  submitWord() {
    ...
    this.selection = "";
    this.game.renderSelection("");
    this.prevLetters.forEach(letter => letter.toggleHighlight());
    this.prevLetters = [];
  }
